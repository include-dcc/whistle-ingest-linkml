...
def pull_values_from_dd(dd_table, writer, empty_writer):
    ...
    for line in csv.DictReader(dd_table):
        ...
        if value_list != "" and value_list is not None and varname not in skip_cols:
            ...
            for value in values:
                ...
                if table_name == coding["table_name"].lower():
                    ...
                    # pdb.set_trace()
                    print(
                        f"Skipping {table_name}.{varname}.{value} because that var is listed under {coding['table_name']}"
                    )
                    print(coding)
...
def pull_codes_from_condition(
    ...
    for line in csv.DictReader(condition_file):
        ...
        try:
            ...
            if coding:
                ...
                if key not in observed_codes:
                    write_coding(
                        writer, condition_desc, coding, "HPO Code", line["HPO Label"]
                    )
        ...
        try:
            ...
            if coding:
                ...
                if key not in observed_codes:
                    write_coding(
                        writer, condition_desc, coding, "MAXO Code", line["MAXO Label"]
                    )
    ...
    sys.stderr.write(
        f"{mismatch_count} labels differed from those found in "
    ...
    def load(self, filename=None):
        ...
        with filename.open("rt") as f:
            ...
            for line in reader:
                # print(line)
                # pdb.set_trace()
                self.coding_list[line["parent_varname"].lower()][
                    line["local code"].lower()
                ] = line
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Build out a baseline harmony csv based "
    ...
    parser.add_argument(
        ...
        type=FileType("wt"),
        help="Name of log to report all codes used that are " "marked as obsolete",
    )
    parser.add_argument(
        "-o",
        "--output-filename",
    ...
    for f in args.config:
        ...
        # We need these for the various terms
        condition_filename = config["dataset"]["condition"]["filename"]

        dds_of_interest = ["condition", "participant", "specimen", "file_manifest"]
        table_dd = []
        for table in dds_of_interest:
            table_dd.append(config["dataset"][table]["data_dictionary"]["filename"])

        outfilename = Path(
            args.output_filename.replace(
                "-harmony.csv", f"-{config['study_id'].lower()}-harmony.csv"
            )
        )
        empties = Path(
            args.unmatched_filename.replace(
                "-harmony.csv", f"-{config['study_id'].lower()}-harmony.csv"
            )
        )
        ...
        with outfilename.open("wt") as outf:
            ...
            with empties.open("wt") as emptyf:
                ...
                for dd_table in table_dd:
                    pull_values_from_dd(open(dd_table, "rt"), writer, empty_writer)

                pull_codes_from_condition(
                    open(condition_filename, "rt"),
                    writer,
                    args.mismatched_report,
                    args.invalid_report,
                    args.obsolete_report,
                )
